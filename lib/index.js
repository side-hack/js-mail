	...
	_createClass(Mail, [{
		...
		value: function render() {
			...
			if (template) {
				var path = template;
				var options = {};
				if (_lodash2.default.isObject(template)) {
					path = template.path;
					options = template.options || {};
				}
				var engine = new _emailTemplates.EmailTemplate(path, options);
		...
		value: function send() {
			...
			var template = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
		...
		get: function get() {
			...
			if (_lodash2.default.isFunction(transport)) {
				...
				var transportClass = new classes[transport.name](transport.options || {});
